...
--------------------------------------------------------------------------------
-- |
-- Module      :  Payasan.PSC.Backend.Csound.RenderScore
...
renderScore specs p1 = 
    vcrush $ map (renderPart1 specs) $ groupByEventBody sameInst p1
     
-- At this point all events should be generated by the same 
-- instrument
--
renderPart1 :: ColumnSpecs -> Part Seconds ScoEvent -> Doc
renderPart1 specs (Part xs) = step xs
  where
    step []     = P.empty
    step (e:es) =    columnHeaders (inst_num $ event_body e) specs 
                  $+$ initialEvent specs e 
                  $+$ restEvents specs e es
